...
#   64   → ~1.3ms   (ultra-low latency, modern hardware)
#   128  → ~2.7ms   (very low latency)
#   256  → ~5.3ms   (balanced)
#   512  → ~10.7ms  (safe fallback)
readonly AUDIO_BUFFER_SIZE="128"

# Audio sample rate (Hz). Match to your .wav files for best results.
#   44100 → CD quality (recommended, matches audio_pack_1 files)
#   48000 → Standard (matches PipeWire default)
...
VOLUME           = int(os.environ.get('WC_VOLUME', '100'))
...
print(f"{C_BLUE}[AUDIO]{C_RESET} Buffer={BUFFER_SIZE} samples (~{latency_ms:.1f}ms) | "
      f"Rate={SAMPLE_RATE}Hz | Channels={MIX_CHANNELS} | Volume={VOLUME}%")
...
VOLUME_FILE = os.path.join(CONFIG_DIR, "volume")
ALL_SOUNDS = list(SOUNDS.values())
...
# === VOLUME WATCHER ===
async def volume_watcher(stop):
    while not stop.is_set():
        try:
            if os.path.exists(VOLUME_FILE):
                with open(VOLUME_FILE) as _vf:
                    _raw = _vf.read().strip()
                if _raw:
                    _new = int(_raw)
                    if _new != CURRENT_VOLUME[0]:
                        CURRENT_VOLUME[0] = _new
                        _v = _new / 100.0
                        for _snd in ALL_SOUNDS:
                            _snd.set_volume(_v)
                        print(f"{C_BLUE}[VOLUME]{C_RESET} {_new}%")
