...
# instead of failing silently (the bar ships from the SCROW setup build).
if [[ "$WB" != /* ]] && ! command -v waybar >/dev/null 2>&1; then
    {
        ...
        if [[ -n "${XDG_CACHE_HOME:-$HOME/.cache}/scrow" ]]; then
            echo "Likely cause: the cava-enabled waybar build failed during SCROW setup."
            echo "Check: ${XDG_CACHE_HOME:-$HOME/.cache}/scrow/waybar-cava-build.log"
