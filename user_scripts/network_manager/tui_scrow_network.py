...
_SCROW_TUI_ROOT = Path.home() / "user_scripts" / "scrow_tui"
...
ENGINE_TYPE = "network"
TARGET_FILE = "~/.cache/scrow_tui/wifi_cache.json"
APP_TITLE = "Scrow Network Manager"
DEFAULT_MODE = "auto"
THEME_FILE = "~/.config/matugen/generated/scrow_tui.json"
...
cache_path = Path.home() / ".cache" / "scrow_tui" / "wifi_cache.json"
...
if __name__ == "__main__":
    ...
    main_router = Path.home() / "user_scripts" / "scrow_tui" / "python" / "main" / "main.py"

    if main_router.exists():
        sys.exit(subprocess.run([sys.executable, str(main_router), str(script_path)] + sys.argv[1:]).returncode)
    else:
        print(f"[-] Error: Main Scrow TUI router not found at {main_router}", file=sys.stderr)
        sys.exit(1)
