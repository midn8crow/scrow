#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Scrow TUI Engine - Generic Configuration Template v5.9.1
...
declare CONFIG_FILE="${SCROW_CONFIG_FILE:-${XDG_CONFIG_HOME}/myapp/settings.conf}"
...
resolve_write_target() {
    ...
    local lock_dir="${XDG_RUNTIME_DIR:-/tmp}/scrow_tui_locks_${USER:-$UID}"
...
create_temp_near() {
    ...
    if ! REPLY=$(mktemp --tmpdir="$target_dir" ".${target_base}.${purpose}.XXXXXXXXXX" 2>/dev/null); then
        if ! REPLY=$(mktemp -t "scrow.${target_base}.${purpose}.XXXXXXXXXX" 2>/dev/null); then
            REPLY=""
            return 1
        fi
    fi
