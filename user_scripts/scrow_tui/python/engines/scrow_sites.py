#!/usr/bin/env python3
"""
🦊 Scrow Sites Engine for Scrow TUI (Arch Linux / Python 3.12+)
================================================================
Engine for managing Scrow Sites per-site theming configuration and 
templates in ~/.config/scrow_sites/ and ~/.config/scrow/settings/scrow_sites/config.json.
"""
...
class ScrowSitesEngine(BaseEngine):
    """
    Engine for managing Scrow Sites configuration and site-specific CSS themes.
    """

    def __init__(self, config_path: str = "~/.config/scrow/settings/scrow_sites/config.json"):
        self.config_path = Path(config_path).expanduser().resolve()
        self.sites_dir = Path("~/.config/scrow_sites").expanduser().resolve()
    ...
    def _read_config_json(self) -> dict[str, Any]:
        if not self.config_path.exists():
            return {
                "colorsPath": "~/.config/matugen/generated/scrow_sites.css",
                "websitesDir": "~/.config/scrow_sites",
    ...
    def load_state(self) -> dict[str, Any]:
        with self._lock:
            ...
            self.cache = {
                ...
                "colorsPath": data.get("colorsPath", "~/.config/matugen/generated/scrow_sites.css"),
                "websitesDir": data.get("websitesDir", "~/.config/scrow_sites"),
            }
