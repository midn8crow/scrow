...
STATE_FILE = Path("/dev/shm/scrow_rapl_state.json")
...
class PkgThrottleEngine(BaseEngine):
    ...
    def save_persistent_state(self):
        try:
            home = get_user_home()
            config_dir = home / ".config" / "scrow" / "settings"
            config_dir.mkdir(parents=True, exist_ok=True)
            state_file = config_dir / "scrow_pkg_power"
    ...
    def restore_state(self) -> bool:
        ...
        try:
            home = get_user_home()
            state_file = home / ".config" / "scrow" / "settings" / "scrow_pkg_power"
