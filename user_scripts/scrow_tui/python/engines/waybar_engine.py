...
class WaybarEngine(BaseEngine):
    def __init__(self, config_path: str = "~/.config/waybar"):
        ...
        state_dir = Path("~/.config/scrow/settings/waybar").expanduser().resolve()
        state_dir.mkdir(parents=True, exist_ok=True)
        self.state_file = state_dir / ".scrow_waybar_state.json"
    ...
    async def _async_restart_waybar(self, target_dir: Path, set_sid: bool = True):
        ...
        lock_file = runtime_dir / "scrow_waybar_restart.lock"
        ...
        try:
            ...
            # 2. Launch Waybar via scrow-run (if available) or fallback to waybar
            cmd = ["scrow-run", "waybar"] if shutil.which("scrow-run") else ["waybar"]
            try:
                subprocess.Popen(
                    cmd,
