#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Dusky ScrowClick Configurator - TUI v1.0.0
# -----------------------------------------------------------------------------
# Target: Arch Linux / Hyprland / UWSM / Wayland
# Engine: Dusky TUI Engine v3.9.5 (Hardened)
#
# Edits the ScrowClick Elite launcher script's configuration variables in-place.
# Changes take effect on the next ScrowClick launch (toggle off → on).
#
# Special handling:
...
#   - Dynamic audio pack discovery from ~/.config/scrowclick/ subdirectories
...
declare -r CONFIG_FILE="${HOME}/user_scripts/scrowclick/dusky_scrowclick.sh"
declare -r APP_TITLE="Dusky ScrowClick Configurator"
...
register_items() {
    ...
    register 0 "Buffer Size"    'AUDIO_BUFFER_SIZE|cycle||64,128,256,512||'              "64"
    register 0 "Sample Rate"    'AUDIO_SAMPLE_RATE|cycle||44100,48000||'              "48000"
    register 0 "Mix Channels"   'AUDIO_MIX_CHANNELS|cycle||8,16,32,64||'             "32"
    ...
    register 1 "Hotplug Poll (sec)"   'HOTPLUG_POLL_SECONDS|float||0.3|5.0|0.1'     "0.5"
...
# Post-Write Hook
# ScrowClick is a toggle launcher — changes take effect on next start.
...
populate_config_cache() {
    ...
    while IFS='=' read -r key_part value_part || [[ -n "${key_part:-}" ]]; do
        ...
        /=/ {
            ...
            # since all ScrowClick config values are simple quoted scalars)
            clean = line
            sub(/[[:space:]]+#.*$/, "", clean)
