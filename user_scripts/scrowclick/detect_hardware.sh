#!/usr/bin/env bash
# ==============================================================================
# SCROWCLICK DEVICE INSPECTOR
# ==============================================================================
# Capability-based input device classifier with ScrowClick behavior prediction.
# Uses ScrowClick's own native-compiled evdev when available.
# ==============================================================================

set -euo pipefail

# --- Find Python with evdev ---
# Priority: ScrowClick's native venv (already compiled) → system python-evdev
WC_PY="$HOME/contained_apps/uv/scrowclick/.venv/bin/python"
...
if [[ -x "$WC_PY" ]] && "$WC_PY" -c "import evdev" &>/dev/null; then
    ...
    printf "  Option 1: \033[1;36m./scrowclick.sh --setup\033[0m  (builds native evdev)\n"
...
# ScrowClick's default EXCLUDED_KEYWORDS (must match scrowclick.sh for accurate prediction)
...
def classify(dev):
    ...
    # --- ScrowClick behavior prediction ---
    # Replicates ScrowClick's exact filtering logic:
...
print(f" {DIM}ScrowClick prediction: {a} active, {f} filtered, {i} ignored "
