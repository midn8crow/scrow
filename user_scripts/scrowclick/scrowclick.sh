#!/usr/bin/env bash
# ==============================================================================
# SCROWCLICK ELITE - ARCH LINUX / UV OPTIMIZED (GOLDEN EDITION)
# ==============================================================================
...
case "${1:-}" in
    ...
    --help|-h)
        ...
        printf "  (no args)  Start or stop ScrowClick (toggle)\n"
        printf "  --setup    Install dependencies and build the environment only\n"
        printf "  --reset    Stop ScrowClick and delete the environment\n"
        exit 0
        ;;
    "") ;;
    *)
...
# Audio pack: subfolder name inside ~/.config/scrowclick/ containing .wav files.
# Example:  ~/.config/scrowclick/audio_pack_1/click.wav
...
readonly AUDIO_MIX_CHANNELS="32"
...
readonly HOTPLUG_POLL_SECONDS="0.5"
...
readonly CONFIG_DIR="$HOME/.config/scrowclick"
readonly STATE_FILE="$HOME/.config/dusky/settings/scrowclick"
...
notify_user() {
    command -v notify-send >/dev/null 2>&1 && \
        notify-send -t 2000 --app-name="ScrowClick" "ScrowClick Elite" "$1"
}
...
os.environ['SDL_APP_NAME'] = 'ScrowClick'
os.environ['PULSE_PROP'] = 'application.name="ScrowClick"'
...
# === VOLUME CONTROL ===
VOLUME_FILE = os.path.join(os.path.dirname(CONFIG_DIR.rstrip('/')), "scrowclick", "volume")
...
CURRENT_VOLUME = [_load_volume()]

def _apply_volume(vol):
    for snd in SOUNDS.values():
        snd.set_volume(vol)

_apply_volume(CURRENT_VOLUME[0])
...
async def volume_watcher(stop_event):
    while not stop_event.is_set():
        try:
            new_vol = _load_volume()
            if abs(new_vol - CURRENT_VOLUME[0]) > 0.001:
                CURRENT_VOLUME[0] = new_vol
                _apply_volume(new_vol)
        except Exception:
            pass
...
async def main():
    ...
    vol_task = asyncio.create_task(volume_watcher(stop))
    print(f"{C_BLUE}[CORE]{C_RESET}  Volume watcher active (file: {VOLUME_FILE})")
    ...
    for t in monitored_tasks.values():
        t.cancel()
    if monitored_tasks:
        await asyncio.gather(*monitored_tasks.values(), vol_task, return_exceptions=True)
    pygame.mixer.quit()
...
if [[ "$RUN_MODE" == "setup" ]]; then
    printf "\n%b[SETUP]%b Setup complete! Run '%b%s%b' to start ScrowClick.\n" \
...
if [[ ! -f "${CONFIG_DIR}/${AUDIO_PACK}/config.json" ]]; then
    if $INTERACTIVE; then
        ...
        notify_user "Missing config.json in ~/.config/scrowclick/${AUDIO_PACK}. Run in terminal."
        exit 1
    fi
fi
...
# --- EXECUTION ---
# Initialize volume file if missing
VOLUME_FILE_INIT="$HOME/.config/scrowclick/volume"
if [[ ! -f "$VOLUME_FILE_INIT" ]]; then
    printf "1.0\n" > "$VOLUME_FILE_INIT"
fi
