...
local ws_layouts = {}
local ws_layout_rules = {}

local state_dir = os.getenv("HOME") .. "/.cache/hypr"
local state_file = state_dir .. "/ws-layouts.lua"

local function load_layout_state()
    ...
    local ok2, saved = pcall(chunk)
    if not ok2 or type(saved) ~= "table" then return end
    ws_layouts = saved
    for id, layout in pairs(ws_layouts) do
        ws_layout_rules[id] = hl.workspace_rule({ workspace = id, layout = layout })
    end
end
...
local function save_layout_state()
    ...
    local parts = {}
    for id, layout in pairs(ws_layouts) do
        parts[#parts + 1] = string.format('["%s"] = "%s"', id, layout)
    end
    local f = io.open(state_file, "w")
    if not f then return end
    f:write("-- autogenerated by hyprland.lua; do not edit\nreturn {\n")
...
-- Scrolling layout: when the focused window has columns on both its left and
-- right, scroll so it sits centered; edge windows keep the default fit.
hl.on("window.active", function(w, reason)
    pcall(function()
        if reason == 1 then return end
        local ws = hl.get_active_workspace()
        ...
        if w.fullscreen and w.fullscreen ~= 0 then return end
        local wx = w.at and (w.at.x or w.at[1])
        local ww = w.size and (w.size.x or w.size[1])
        if not wx or not ww then return end
        local left, right = false, false
        for _, o in ipairs(hl.get_windows()) do
            if o.address ~= w.address and not o.floating and o.workspace and o.workspace.id == ws.id then
                local ox = o.at and (o.at.x or o.at[1])
                if ox then
                    if ox < wx then left = true end
                    if ox > wx then right = true end
                end
            end
        end
        if not (left and right) then return end
        local mon = w.monitor or hl.get_active_monitor()
        if not mon or not mon.width then return end
        local res = mon.reserved or {}
        local center = (mon.x or 0) + (res.left or 0) + (mon.width - (res.left or 0) - (res.right or 0)) / 2
        local delta = center - (wx + ww / 2)
...
local pending_focus_restore = nil
hl.on("config.props_refreshed", function()
    if not pending_focus_restore then
        return
    end
    local addr = pending_focus_restore
    pending_focus_restore = nil
    if addr then
        hl.dispatch(hl.dsp.focus({ window = "address:" .. addr }))
    end
end)
hl.bind(mainMod .. " + SHIFT + L", function()
    ...
    if cur and cur.address then
        pending_focus_restore = cur.address
    end
    if ws_layout_rules[id] then
        ws_layout_rules[id]:set_enabled(false)
    end
    ws_layout_rules[id] = hl.workspace_rule({ workspace = id, layout = ws_layouts[id] })
    save_layout_state()
end)
